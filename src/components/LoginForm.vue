<template>
  <div class="container login-page mt-5">
    <header class="d-flex flex-column justify-content-center align-items-center mb-4">
      ...
      <h3 class="quizz-app-subtitle">Login to access your account!</h3>
    </header>
    <div class="d-flex justify-content-center align-items-center content-wrapper">
      ...
      <form @submit.prevent="handleLogin" class="login ms-4">
        ...
        <div class="login-options mb-4">
          <label class="text-light me-3">
            <input type="radio" v-model="loginType" value="user" required> Login as User
          </label>
          <label class="text-light">
            <input type="radio" v-model="loginType" value="admin" required> Login as Admin
          </label>
        </div>
        <input type="submit" value="Login" class="btn btn-custom btn-lg">
...
const email = ref('');
const password = ref('');
const loginType = ref('user'); // Default to 'user'
const { error, login } = useLogin();
const router = useRouter();

const handleLogin = async () => {
  console.log("Attempting login with email:", email.value, "as", loginType.value);
  const user = await login(email.value, password.value, loginType.value);
  if (!error.value) {
    console.log("Login successful, user data:", user);
    if (loginType.value === 'admin' && user.isAdmin) {
      router.push('/admin');
    } else {
      router.push('/home');
    }
...
.app-logo {
  width: 100%;
  height: auto;
  max-height: 500px; /* Ensure the image has the same height as the login form */
  border-radius: 10px;
}
...
.login {
  ...
  border: 2px solid #fff; /* White border for login form */
}
...
.btn-custom:hover {
  background-color: #56CCF2; /* Slightly lighter shade for hover */
}
